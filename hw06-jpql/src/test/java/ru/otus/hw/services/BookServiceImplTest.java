...
class BookServiceImplTest {
    ...
    void shouldHaveCreateListBooksByIdWithMethod() {
        ...
        for (int i = 0; i < actualBooks.size(); i++) {
            ...
            System.out.println(bookActual.getAuthor().getFullName());
            System.out.println(bookExpected.getAuthor().getFullName());

            assertAll("bookActual",
                    () -> assertEquals(bookActual.getId(), bookExpected.getId()),
                    () -> assertEquals(bookActual.getTitle(), bookExpected.getTitle()),
                    () -> assertEquals(bookActual.getAuthor().getId(), bookExpected.getAuthor().getId()),
                    () -> assertEquals(bookActual.getAuthor().getFullName(), bookExpected.getAuthor().getFullName()),
                    () -> assertEquals(bookActual.getGenre().getId(), bookExpected.getGenre().getId()),
                    () -> assertEquals(bookActual.getGenre().getName(), bookExpected.getGenre().getName()));
        }
    }
